# Leah พาเพลิน (Leah-PP) · คำสั่งประจำโปรเจกต์สำหรับ AI ที่เขียนโค้ด
...
- **v24: หมู่บ้านเป็น 2D แล้ว** (ส่วน `5b. VIEW 2D` ใน `index.html` · ผัง 26×17 ช่องจาก proto2d อยู่ใน `VL`) · ถ้ำและในบ้านยังเป็น 3D รอเปลี่ยนต่อ
...
## งานถัดไป
เปลี่ยนส่วนวาดภาพ (VIEW) ของ `index.html` เป็น 2D canvas ตามแนว `proto2d.html` ทีละฉาก: ~~หมู่บ้าน~~ (เสร็จ v24) → ถ้ำ → ในบ้าน
- ฉากใหม่ใช้ของใน `5b. VIEW 2D` ซ้ำได้: `v2Measure` (หาจุดแตะพื้น + ปลูกหญ้ากลบฐาน) `v2Shade` (เงาชั้นเดียว) `v2Char` (เงาตัวละครเบลอล่วงหน้า) `v2Draw`
...
- เตรียม: `npm pack three@0.128.0 && tar xzf three-0.128.0.tgz` ที่ root (ได้โฟลเดอร์ `package/`) · เปิด `python3 -m http.server 8799` ที่ root · รันใน `tests/` เช่น `cd tests && python3 test12.py` (เปลี่ยนพอร์ตด้วย `PORT=xxxx`, path three.js ด้วย `THREE_JS=...`)
- ไฟล์: `test20.py` (แตะอาคาร/คนในหมู่บ้าน 2D) `test_walk.py` (เดินจริงทุกฉาก) `test12.py` (วงจรหลัก) `test14.py` (ที่ซ่อน) `test15.py` (บทพูด) `test16.py` (สมุด) `test18.py` (จอย/ปุ่ม) `test19.py` (เพลง)
