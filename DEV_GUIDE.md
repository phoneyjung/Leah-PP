...
## จุดทดสอบ (window.PP)
`PP.S` สถานะผู้เล่น · `PP.M` แผนที่ปัจจุบัน · `PP.CRYS` คริสตัลรอบนี้ · `PP.player` · `PP.interact(obj)` · `PP.goScene(name,'start',true)` · `PP.command(type,data)` · `PP.quiz` · `PP.nextLine()` / `PP.dlgOpen` · `PP.isWall(x,z)` · `PP.JOY` · `PP.MUSIC` · `PP.QB`
